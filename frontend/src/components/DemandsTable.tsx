...
export default function DemandsTable({ 
  ...
  // Definir as colunas da tabela com tipagem correta
  const columns = [
    {
      accessorKey: 'type',
      header: 'Tipo',
    ...
    },
    {
      ...
      header: 'Data',
      cell: ({ getValue }: any) => new Date(getValue()).toLocaleDateString('pt-BR'),
    },
    {
      id: 'actions',
      header: 'Ações',
            ...
            {table.getHeaderGroups().map(headerGroup => (
              <tr key={headerGroup.id}>
                {headerGroup.headers.map(header => (
                  <th 
                    key={header.id}
                    className="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-150"
              ...
              table.getRowModel().rows.map(row => (
                ...
                >
                  {row.getVisibleCells().map(cell => (
                    <td key={cell.id} className="px-6 py-5 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">
