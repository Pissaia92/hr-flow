...
interface EnhancedChartsMockProps {
  useMockData?: boolean;
}

export default function EnhancedChartsMock({ useMockData = true }: EnhancedChartsMockProps) {
  // Função para gerar dados simulados
  const generateMockMetrics = () => {
    // Dados simulados para o período 03/25 até 08/25
    const evolution = [
    ...
    // Calcular totais baseados na evolução
    const total = evolution.reduce((sum, month) => sum + month.count, 0);
    
    // Distribuição de prioridades (baseada no total)
    const byPriority = {
      urgent: Math.floor(total * 0.15),      // 15% urgentes
      important: Math.floor(total * 0.35),   // 35% importantes
      normal: total - Math.floor(total * 0.15) - Math.floor(total * 0.35) // Restante normal
    };

    // Distribuição de status (baseada no total)
    const byStatus = {
      open: Math.floor(total * 0.25),        // 25% abertas
      in_progress: Math.floor(total * 0.35), // 35% em progresso
      closed: total - Math.floor(total * 0.25) - Math.floor(total * 0.35) // Restante fechadas
    };
  ...
  const mockMetrics = useMockData ? generateMockMetrics() : null;

  if (!mockMetrics) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
        <p className="text-gray-500 dark:text-gray-400">Dados não disponíveis</p>
      </div>
    );
  }
  ...
  const priorityData = [
    { name: 'Urgent', value: mockMetrics.byPriority.urgent, color: COLORS.danger },
    { name: 'Important', value: mockMetrics.byPriority.important, color: COLORS.warning },
    { name: 'Normal', value: mockMetrics.byPriority.normal, color: COLORS.success }
  ];

  // Dados para gráficos de status
  const statusData = [
    { name: 'Open', value: mockMetrics.byStatus.open, color: COLORS.primary },
    { name: 'In progress', value: mockMetrics.byStatus.in_progress, color: COLORS.warning },
    { name: 'Closed', value: mockMetrics.byStatus.closed, color: COLORS.success }
  ];

  // Dados para evolução temporal (últimos 6 meses)
  const evolutionData = mockMetrics.evolution || [];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 p-6 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Demands total</p>
              <p className="text-3xl font-bold text-gray-900 dark:text-white mt-2">{mockMetrics.total}</p>
        ...
        <div className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 p-6 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Urgent Demands</p>
              <p className="text-3xl font-bold text-red-600 dark:text-red-400 mt-2">{mockMetrics.byPriority.urgent}</p>
        ...
        <div className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 p-6 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-3xl font-bold text-yellow-600 dark:text-yellow-400 mt-2">{mockMetrics.byStatus.in_progress}</p>
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-3xl font-bold text-green-600 dark:text-green-400 mt-2">
                {mockMetrics.byStatus.closed ? Math.round((mockMetrics.byStatus.closed / mockMetrics.total) * 100) : 0}%
              </p>
              <p className="text-xs text-green-600 dark:text-green-400 mt-1">✅ Concluded</p>
            </div>
