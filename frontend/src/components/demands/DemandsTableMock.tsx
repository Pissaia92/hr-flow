...
interface Demand {
  ...
  created_at: string;
  user_id: string;
  user_name?: string; // Adicionado para exibição do nome do usuário
}

interface DemandsTableMockProps {
  // Este componente não recebe mais dados externos, ele os gera internamente
}

export default function DemandsTableMock({}: DemandsTableMockProps) {
  ...
  const [demands, setDemands] = useState<Demand[]>([]); // Estado para os dados simulados

  // Gerar dados simulados ao montar o componente
  useEffect(() => {
    const mockDemands = generateMockDemands();
    setDemands(mockDemands);
  }, []);

  // Função para gerar dados simulados
  const generateMockDemands = (): Demand[] => {
    const demandTypes = [
        'Vacation Request',
        'Equipment Request',
    ...
    const users = [
      ...
      { id: '00000000-0000-0000-0000-000000000003', name: 'Pedro Costa (Teste)' }
    ];

    const mockDemands: Demand[] = [];
    
    // Gerar 35 demandas simuladas
    for (let i = 1; i <= 35; i++) {
      const randomType = demandTypes[Math.floor(Math.random() * demandTypes.length)];
      ...
      mockDemands.push({
        id: `demand-${i.toString().padStart(4, '0')}`,
        type: randomType,
        description: `${randomType} - Solicitation #${i}`,
          ...
          <button
            onClick={() => {
              if (confirm('Are you sure?')) {
                // Em modo mock, apenas simular a exclusão
                alert('Demand excluded (simulation)!');
                // Para implementação real, você chamaria uma API de deleção aqui
              }
            }}
  ...
  const table = useReactTable<Demand>({
    data: demands, // Usar os dados simulados
    columns,
    state: {
      sorting,
      columnFilters,
    ...
    <div className="bg-white dark:bg-gray-800 shadow-xl rounded-2xl overflow-hidden">
      ...
      <div className="flex justify-between items-center p-6">
        <h3 className="text-lg font-medium text-gray-900 dark:text-white">
          Demands list (For tests purpose)
        </h3>
        <ExportButtons data={demands} filename="demands-hrflow-mock" />
      </div>
                ...
                <td colSpan={columns.length} className="px-6 py-12 text-center">
                  <div className="flex flex-col items-center justify-center">
                    ...
                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                      Try adjusting filters or create a new demand
                    </p>
                  </div>
                </td>
              </tr>
